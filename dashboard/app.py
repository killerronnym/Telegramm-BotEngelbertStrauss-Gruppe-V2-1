...
QUIZ_DATA_FILE = os.path.join(DATA_DIR, "quizfragen.json")
QUIZ_SENT_FILE = os.path.join(DATA_DIR, "quizfragen_sent.json")
QUIZ_LOCK_FILE = os.path.join(DATA_DIR, "quizfragen.lock")

UMFRAGE_DATA_FILE = os.path.join(DATA_DIR, "umfragen.json")
UMFRAGE_SENT_FILE = os.path.join(DATA_DIR, "umfragen_sent.json")
UMFRAGE_LOCK_FILE = os.path.join(DATA_DIR, "umfragen.lock")
...
AVAILABLE_PERMISSIONS = {
    k: v for group in AVAILABLE_PERMISSION_GROUPS.values() for k, v in group.items()
}
...
def file_lock(lock_path: str):
    """Einfaches Prozess-Lock (Linux/Synology)."""
...
def _extract_list(data, key: str) -> list:
    """Unterstützt beide Formate: dict mit key ODER Liste direkt."""
...
def _quiz_fp(q: dict) -> str:
    frage = _norm_text(str(q.get("frage", "")))
    optionen = q.get("optionen", []) or []
    optionen_norm = [_norm_text(str(x)) for x in optionen]
    antwort = str(q.get("antwort", ""))
    payload = json.dumps(
        {"frage": frage, "optionen": optionen_norm, "antwort": antwort},
        ensure_ascii=False,
        separators=(",", ":"),
    )
...
def _poll_fp(p: dict) -> str:
    frage = _norm_text(str(p.get("frage", "")))
    optionen = p.get("optionen", []) or []
    optionen_norm = [_norm_text(str(x)) for x in optionen]
    payload = json.dumps(
        {"frage": frage, "optionen": optionen_norm},
        ensure_ascii=False,
        separators=(",", ":"),
    )
    return hashlib.sha256(payload.encode("utf-8")).hexdigest()
...
def _get_sent_fps(sent_file: str, key: str = "sent") -> set:
    raw = load_json(sent_file, {key: []})
    ...
    if not isinstance(items, list):
        return set()
    return {it.get("_fp") for it in items if isinstance(it, dict) and it.get("_fp")}


def mark_quiz_question_sent_and_remove_from_pool(q: dict) -> None:
    fp = _quiz_fp(q)
    with file_lock(QUIZ_LOCK_FILE):
        pool_raw = load_json(QUIZ_DATA_FILE, {"questions": []})
        pool_is_dict = isinstance(pool_raw, dict)
        questions = _extract_list(pool_raw, "questions")

        sent_fps = _get_sent_fps(QUIZ_SENT_FILE, key="sent")
        sent_raw = load_json(QUIZ_SENT_FILE, {"sent": []})
        sent_list = (
            sent_raw.get("sent", [])
            if isinstance(sent_raw, dict)
            else (sent_raw if isinstance(sent_raw, list) else [])
        )
        ...
        if fp not in sent_fps:
            saved = dict(q)
            saved["_fp"] = fp
            saved["_sent_at"] = datetime.utcnow().isoformat() + "Z"
            sent_list.append(saved)
        ...
        if pool_is_dict:
            pool_out = dict(pool_raw)
            pool_out["questions"] = new_questions
        else:
            pool_out = new_questions

        save_json_atomic(QUIZ_DATA_FILE, pool_out)
        save_json_atomic(QUIZ_SENT_FILE, {"sent": sent_list})

        log.info(
            "QUIZ MOVE OK | removed=%s | pool_now=%s | sent_now=%s | pool_file=%s | sent_file=%s",
            removed, len(new_questions), len(sent_list), QUIZ_DATA_FILE, QUIZ_SENT_FILE
        )


def mark_poll_sent_and_remove_from_pool(p: dict) -> None:
    """Umfragen genau wie Quiz: sent schreiben + aus Pool entfernen."""
    fp = _poll_fp(p)
    with file_lock(UMFRAGE_LOCK_FILE):
        pool_raw = load_json(UMFRAGE_DATA_FILE, {"polls": []})
        pool_is_dict = isinstance(pool_raw, dict)
        polls = _extract_list(pool_raw, "polls")

        sent_fps = _get_sent_fps(UMFRAGE_SENT_FILE, key="sent")
        sent_raw = load_json(UMFRAGE_SENT_FILE, {"sent": []})
        sent_list = (
            sent_raw.get("sent", [])
            if isinstance(sent_raw, dict)
            else (sent_raw if isinstance(sent_raw, list) else [])
        ...
        for item in polls:
            try:
                if _poll_fp(item) == fp:
                    ...
                    new_polls.append(item)
            except Exception:
                new_polls.append(item)

        if pool_is_dict:
            pool_out = dict(pool_raw)
            pool_out["polls"] = new_polls
        else:
            pool_out = new_polls

        save_json_atomic(UMFRAGE_DATA_FILE, pool_out)
        save_json_atomic(UMFRAGE_SENT_FILE, {"sent": sent_list})

        log.info(
            "POLL MOVE OK | removed=%s | pool_now=%s | sent_now=%s | pool_file=%s | sent_file=%s",
            removed, len(new_polls), len(sent_list), UMFRAGE_DATA_FILE, UMFRAGE_SENT_FILE
        )
    ...
    config.update(
        {
            ...
            "TEMPORARY_MESSAGE_DURATION_SECONDS": int(
                form.get("TEMPORARY_MESSAGE_DURATION_SECONDS", 30)
            ),
        }
    )
...
def outfit_bot_start_contest():
    if is_outfit_bot_running():
        trigger_path = os.path.join(OUTFIT_BOT_DIR, "command_start_contest.tmp")
...
def outfit_bot_announce_winner():
    if is_outfit_bot_running():
        trigger_path = os.path.join(OUTFIT_BOT_DIR, "command_announce_winner.tmp")
        with open(trigger_path, "w", encoding="utf-8") as f:
            f.write("trigger")
...
def outfit_bot_end_duel():
    log.debug("/outfit-bot/end-duel wurde aufgerufen")
    ...
    if is_outfit_bot_running():
        ...
        log.warning("Duell beenden angefordert, aber Bot läuft nicht.")
...
def bot_settings():
    if request.method == "POST":
        ...
        if action == "start_invite_bot":
            ...
            config["repost_profile_for_existing_members"] = (
                "repost_profile_for_existing_members" in request.form
            )
            try:
...
def quiz_settings():
    if request.method == "POST":
        ...
        if action == "save_settings":
            ...
            try:
                with open(DASHBOARD_APP_LOG, "w", encoding="utf-8") as f:
                    f.write("")
    ...
    dashboard_config = load_json(DASHBOARD_CONFIG_FILE, {})
    logs = get_bot_logs(DASHBOARD_APP_LOG, lines=50)
...
def send_random_quiz_route():
    ...
    quiz_cfg = config.get("quiz", {}) if isinstance(config, dict) else {}
    ...
    log.info("QUIZ SEND | QUIZ_DATA_FILE=%s | QUIZ_SENT_FILE=%s", QUIZ_DATA_FILE, QUIZ_SENT_FILE)

    data = load_json(QUIZ_DATA_FILE, {"questions": []})
    questions = _extract_list(data, "questions")
    ...
    for item in questions:
        try:
            if _quiz_fp(item) not in sent_fps:
                available.append(item)
        except Exception:
            pass

    if not available:
        flash("⚠️ Es sind keine neuen Quizfragen mehr übrig (alles wurde schon gesendet).", "warning")
        return redirect(url_for("quiz_settings"))
    ...
    try:
        correct = int(q.get("antwort", 0))
    except Exception:
        correct = 0

    if not question or not isinstance(options, list) or len(options) < 2:
        flash("Ungültige Quizfrage: Frage oder Optionen fehlen (mind. 2 Optionen nötig).", "danger")
        return redirect(url_for("quiz_settings"))
    ...
    if success:
        try:
            mark_quiz_question_sent_and_remove_from_pool(q)
        except Exception as e:
            log.error("Quiz wurde gesendet, aber Move/Dedupe ist fehlgeschlagen: %s", e)
            flash("Quiz gesendet ✅, aber Speichern/Entfernen ist fehlgeschlagen (siehe Logs).", "warning")
            return redirect(url_for("quiz_settings"))
...
# --- UMFRAGE SENDER ROUTE (wie Quiz) ---
@app.route("/umfrage/send-random", methods=["POST"])
def send_random_poll_route():
    config = load_json(DASHBOARD_CONFIG_FILE, {})
    poll_cfg = config.get("umfrage", {}) if isinstance(config, dict) else {}
    ...
    log.info("POLL SEND | UMFRAGE_DATA_FILE=%s | UMFRAGE_SENT_FILE=%s", UMFRAGE_DATA_FILE, UMFRAGE_SENT_FILE)

    data = load_json(UMFRAGE_DATA_FILE, {"polls": []})
    polls = _extract_list(data, "polls")
    ...
    for item in polls:
        try:
            if _poll_fp(item) not in sent_fps:
                available.append(item)
        except Exception:
            pass

    if not available:
        flash("⚠️ Es sind keine neuen Umfragen mehr übrig (alles wurde schon gesendet).", "warning")
        return redirect(url_for("umfrage_settings"))
    ...
    if not question or not isinstance(options, list) or len(options) < 2:
        flash("Ungültige Umfrage: Frage oder Optionen fehlen (mind. 2 Optionen nötig).", "danger")
        return redirect(url_for("umfrage_settings"))
    ...
    if success:
        try:
            mark_poll_sent_and_remove_from_pool(p)
        except Exception as e:
            log.error("Umfrage wurde gesendet, aber Move/Dedupe ist fehlgeschlagen: %s", e)
            flash("Umfrage gesendet ✅, aber Speichern/Entfernen ist fehlgeschlagen (siehe Logs).", "warning")
            return redirect(url_for("umfrage_settings"))
